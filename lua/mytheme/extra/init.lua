...
-- TODO: Configure your theme metadata here
M.theme = {
  name = "My Theme",         -- Display name used in generated configs
  id = "mytheme",            -- Identifier used in filenames
  author = "Your Name",      -- Author name for configs that include it
  repo_url = "",             -- GitHub URL for upstream references
}
...
function M.setup()
  ...
  for _, extra in ipairs(names) do
    ...
    local plugin = require("mytheme.extra." .. extra)
    for style, style_name in pairs(styles) do
      local colors, groups, opts = theme.setup({ style = style, plugins = { all = true } })
      local fname = extra
      ...
      colors["_upstream_url"] = M.theme.repo_url ~= "" and (M.theme.repo_url .. "/raw/main/extras/" .. fname) or ""
      colors["_style_name"] = M.theme.name .. style_name
      colors["_name"] = M.theme.id .. "_" .. style
      colors["_style"] = style
      colors["_author"] = M.theme.author
      print("[write] " .. fname)
